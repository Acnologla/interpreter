...
func visit(intToken interface{}) interface{} {
	...
	if token.Value == "index" {
		...
		if !ok {
			val, ok := visit(token.Right).(string)
			tkn := token.Right
			if ok {
				if !strings.HasPrefix(val, "\"") && !strings.HasSuffix(val, "\"") {
					tkn = &Token{Value: visit(token.Right).(string) }
				}
				return visit(&Token{
					Value: "property",
					Left:  token.Left,
		...
		for {
			...
			if ok {
				if returned.Value == "forif" {
					...
					rightToken, rightOk := returned.Right.(*Token)
					if rightOk{
						if rightToken.Value == "ret"{
							return rightToken
						}
						if rightToken.Value == "break"{
							return lastVal
						}
					}
					if ok {
	...
	if token.Value == "&&"{
		val, ok := visit(token.Left).(bool)
		val2,ok2 := visit(token.Right).(bool)
		if ok{
			if val{
				if ok2{
					if val2{
